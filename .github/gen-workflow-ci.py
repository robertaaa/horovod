...
def main():
    ...
    def build_and_test_images(name: str,
        ...
        return (f'  {name}:\n'
                ...
                f'        run: .github/add-cache-to-docker-compose.sh "${{{{ steps.ecr.outputs.registry }}}}" < docker-compose.test.yml > docker-compose.test.with_cache.yml\n'
                f'        shell: bash\n'
                f'\n'
                ...
                f'        id: build\n'
                f'        run: |\n'
                f'          .github/timeout-and-retry.sh ${{{{ matrix.build_timeout }}}}m 3 10 docker-compose -f docker-compose.test.with_cache.yml build ${{{{ matrix.image }}}}\n'
                f'\n' +
